#!/usr/bin/env python3
"""
Sri Lanka Advance Bookings Scraper (Venue API)
- Uses /showtimes/byvenue endpoint (same as boxoffice)
- Fetches data for TOMORROW (not today)
- Merges with existing daily file (robust retries)
...
- Generates movie‑slug JSON and index.json for advance bookings
"""
...
TIMEOUT_SEC = 30
# No cutoff for advance bookings (keep all shows)
CUT_OFF_MINUTES = 999999          # effectively no cutoff
REGION_CODE = "SNLK"
IST = ZoneInfo("Asia/Kolkata")
BASE_DELAY = 1.0                 # base delay between requests

# Paths – separate from boxoffice
BASE_DIR = "srilanka"
ADVANCE_BOXOFFICE_DIR = os.path.join(BASE_DIR, "advance", "boxoffice")
ADVANCE_MOVIE_DIR = os.path.join(BASE_DIR, "advance", "movie")
ADVANCE_DATA_DIR = os.path.join(ADVANCE_MOVIE_DIR, "data")
os.makedirs(ADVANCE_BOXOFFICE_DIR, exist_ok=True)
os.makedirs(ADVANCE_DATA_DIR, exist_ok=True)

VENUES_FILE = os.path.join(os.path.dirname(__file__), "venues.json")

# User‑Agent pool (identical to boxoffice)
# User‑Agent pool – only iOS devices, randomized selection
USER_AGENTS = [
    ...
    # iPad Pro / Air – iOS 16/17
    "Mozilla/5.0 (iPad; CPU OS 17_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.0 Mobile/15E148 Safari/604.1",
    "Mozilla/5.0 (iPad; CPU OS 16_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.0 Mobile/15E148 Safari/604.1",
    "Mozilla/5.0 (iPad; CPU OS 15_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/15.0 Mobile/15E148 Safari/604.1",
    # Additional variants (iPhone SE, etc.)
    "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.0 Mobile/15E148 Safari/604.1 (compatible; Googlebot/2.1)",
    "Mozilla/5.0 (iPhone; CPU iPhone OS 16_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) CriOS/116.0.5845.119 Mobile/15E148 Safari/604.1",
    "Mozilla/5.0 (iPhone; CPU iPhone OS 15_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) FxiOS/102.0 Mobile/15E148 Safari/605.1.15",
]
# The scraper will randomly pick one from this list for each new session.
...
def get_daily_file_path(date_str):
    ...
    month = date_str[4:6]
    day = date_str[6:8]
    path = os.path.join(ADVANCE_BOXOFFICE_DIR, year, f"{month}-{day}.json")
...
def save_daily_file(date_str, shows_dict, venue_details_map):
    ...
    print(f"💾 Saved advance daily file: {daily_path}")

#########################################
# ADVANCE MOVIE DATABASE UPDATE
#########################################
def update_advance_movie_database():
    """Scans all advance daily files and builds per-movie JSON + index.json."""
    daily_files = []
    for root, dirs, files in os.walk(ADVANCE_BOXOFFICE_DIR):
        for f in files:
            if f.endswith(".json"):
    ...
    for movie_title, date_data in movie_date_data.items():
        ...
        per_movie_path = os.path.join(ADVANCE_DATA_DIR, f"{slug}.json")
    ...
    index_path = os.path.join(ADVANCE_MOVIE_DIR, "index.json")
    atomic_dump(index_path, {
        "last_updated": now_ist_str(),
        "movies": index
    })
    print(f"📊 Advance movie database updated – {len(index)} movies")

#########################################
# MAIN
#########################################
def main():
    print("\n🚀 Sri Lanka Advance Bookings Tracker (Venue API) Started...\n")
    ...
    print(f"📂 Loaded {len(existing_shows)} existing advance shows from daily file")
    ...
    test_date = target_date   # test with tomorrow
    ...
    # No cutoff for advance – keep all shows
    eligible_new = all_new_shows   # all shows are kept

    print(f"\n✅ Total advance shows scraped: {len(all_new_shows)}")
    ...
    print(f"📁 Advance daily file saved: {daily_path}")

    # Update movie database for advance
    update_advance_movie_database()

    print("\n🎉 DONE — ADVANCE BOOKINGS UPDATED | PERMANENT DB ACTIVE | COMPRESSED DAILY FILES\n")
